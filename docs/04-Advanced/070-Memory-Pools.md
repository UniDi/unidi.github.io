...
Note that unlike in other examples, we derive from `PlaceholderFactory`, implement `IDisposable`, and we use `FromMonoPoolableMemoryPool` instead of `FromPoolableMemoryPool`.  Also be careful of potential TODO: <a href="#aot-error">AOT errors</a> with this approach.
...
Static memory pools are especially useful for common data structures such as lists or dictionaries.  UniDi includes some standard memory pools for this exact purpose which you can use.  For example, let's say you are writing a `MonoBehaviour` that needs to iterate over every component on a game object every frame.  You might implement it like this:
...
However, if you run a scene with this MonoBehaviour added to it, and open up the unity profiler, you will see that there is around 48 bytes allocated per frame.  We can get rid of that by using the `UniDi.ListPool` class and doing this instead:
...
To detect these mistakes, UniDi includes an optional class that you can install that will throw exceptions when there are actively spawned objects when the scene is closed.  Just add the following to one of your scene installers:
...
UniDi also includes an experimental editor window that can be used to monitor the sizes of all the memory pools in the scene.  You can open it by clicking `Window -> UniDi Pool Monitor` inside Unity and should look like this:

TODO: link image<!-- <img src="Images/PoolMonitor.png?raw=true" alt="Memory Pool Monitor"/> -->
